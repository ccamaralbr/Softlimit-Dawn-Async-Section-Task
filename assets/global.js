...
// CUSTOM RENDER SECTION ASYNC CODE

// Update page URL w/o refresh
var changeContentUrl = function (event) {
  event.preventDefault();
  event.stopPropagation();
  console.log(this);
  const new_url = this.dataset.url;
  const sectId = this.dataset.section;
  if (window.location.pathname != new_url) {
    history.pushState({}, "", `${window.location.origin}${new_url}`);
  }

  // Update section content w/o refresh, using Async Section Rendering API
  function handleResponse() {
    //                  console.log(Object.values(JSON.parse(this.responseText))[0]);
    document.querySelector(
      `#shopify-section-${sectId}`
    ).parentElement.innerHTML = Object.values(JSON.parse(this.responseText))[0];

    // Reseting listeners after a response / page change
    document
      .querySelectorAll("#target_link")
      .forEach((item) => item.addEventListener("click", changeContentUrl));
  }

  // XHR Request Shopify docs
  const request = new XMLHttpRequest();
  request.addEventListener("load", handleResponse);
  request.open("GET", `${new_url}/?sections=${sectId}`, true);
  request.send();
};

// Setting listeners for all nav links
document
  .querySelectorAll("#target_link")
  .forEach((item) => item.addEventListener("click", changeContentUrl));